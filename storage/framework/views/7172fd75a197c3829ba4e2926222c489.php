...
<div style="width: 76vw; min-height: 100vh; display: flex; flex-direction: column; background: #f8fafc; margin: 0; padding: 0; overflow-x: hidden;">
    <div style="flex: 1; display: flex; flex-direction: column; padding: 20px 0; margin: 0;">
        ...
        <?php
            $pendingPayments = $fines->filter(function ($fine) {
                return $fine->fine_status === 'pending_payment';
            })->values();
        ?>

        <?php if($pendingPayments->isNotEmpty()): ?>
        <div style="background:#fff; padding:24px; border-radius:8px; border:1px solid #e2e8f0; box-shadow:0 1px 3px rgba(0,0,0,.05); margin-bottom: 24px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2 style="margin: 0; font-size: 18px; font-weight: 600;">⏳ Permintaan Pembayaran Denda (Menunggu Persetujuan)</h2>
                <span style="background: #fef3c7; color: #b45309; padding: 6px 12px; border-radius: 6px; font-weight: 600; font-size: 12px;">
                    Total: <?php echo e(count($pendingPayments)); ?> Permintaan
                </span>
            </div>
            ...
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                        <?php $__currentLoopData = $pendingPayments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fine): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr style="border-bottom: 1px solid #e2e8f0;">
                            ...
                            <td style="padding: 12px;">
                                <form method="POST" action="<?php echo e(route('admin.fines.approve-payment', $fine->id)); ?>" style="display: inline;">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit" style="background: #10b981; color: #fff; padding: 6px 12px; border-radius: 6px; border: none; text-decoration: none; font-size: 12px; font-weight: 600; cursor: pointer; margin-right: 8px;">
                                        Setujui
                                    </button>
                                </form>
                                <form method="POST" action="<?php echo e(route('admin.fines.reject-payment', $fine->id)); ?>" style="display: inline;">
                                    <?php echo csrf_field(); ?>
                                    <button type="submit" style="background: #ef4444; color: #fff; padding: 6px 12px; border-radius: 6px; border: none; text-decoration: none; font-size: 12px; font-weight: 600; cursor: pointer;">
                                        Tolak
                                    </button>
                                </form>
                            </td>
        ...
        <!-- DAFTAR DENDA YANG SUDAH DISETUJUI -->
                ...
                <div style="overflow-x: auto;">
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <?php $__currentLoopData = $fines; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fine): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if($fine->getDaysDue() > 0 && $fine->fine_status !== 'pending_payment'): ?>
                                <tr style="border-bottom: 1px solid #e2e8f0;">
                                    <td style="padding: 12px;"><?php echo e($fine->guest_name); ?></td>
                                    <td style="padding: 12px;"><?php echo e($fine->book->name); ?></td>
                                    <td style="padding: 12px;">
                                    ...
                                    <td style="padding: 12px;">
                                        <a href="<?php echo e(route('admin.fines.show', $fine->id)); ?>" style="background: #6366f1; color: #fff; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: 600; display: inline-block;">
                                            Detail
                                        </a>
                                    </td>
                                </tr>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\ukk-peminjaman-buku\resources\views/admin/fines/index.blade.php ENDPATH**/ ?>