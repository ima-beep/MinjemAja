                    ...
                    <tbody>
                        @forelse($members as $member)
                            <tr style="border-bottom: 1px solid #e2e8f0;">
                                ...
                                <td style="padding: 12px;">
                                    <a href="{{ route('teacher.members.show', $member->id) }}" style="background: #6366f1; color: #fff; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: 600; display: inline-block; margin-right: 8px;">
                                        Detail
                                    </a>
                                    <a href="{{ route('teacher.members.edit', $member->id) }}" style="background: #3b82f6; color: #fff; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: 600; display: inline-block; margin-right: 8px;">
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('teacher.members.destroy', $member->id) }}" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
