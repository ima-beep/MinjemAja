@extends('layouts.admin')

@section('title', 'Kelola Denda')

@section('content')
<div style="width: 76vw; min-height: 100vh; display: flex; flex-direction: column; background: #f8fafc; margin: 0; padding: 0; overflow-x: hidden;">
    <div style="flex: 1; display: flex; flex-direction: column; padding: 20px 0; margin: 0;">
        <h1 style="font-size: 28px; font-weight: 700; margin-bottom: 24px;">💰 Kelola Denda</h1>

        @if(session('success'))
            <div style="background: #dcfce7; border: 1px solid #86efac; border-radius: 6px; padding: 12px; margin-bottom: 20px; color: #166534;">
                {{ session('success') }}
            </div>
        @endif

        <!-- PENDING PAYMENT REQUESTS -->
        @php
            $pendingPayments = $fines->filter(function ($fine) {
                return $fine->fine_status === 'pending_payment';
            })->values();
        @endphp

        @if($pendingPayments->isNotEmpty())
        <div style="background:#fff; padding:24px; border-radius:8px; border:1px solid #e2e8f0; box-shadow:0 1px 3px rgba(0,0,0,.05); margin-bottom: 24px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2 style="margin: 0; font-size: 18px; font-weight: 600;">⏳ Permintaan Pembayaran Denda (Menunggu Persetujuan)</h2>
                <span style="background: #fef3c7; color: #b45309; padding: 6px 12px; border-radius: 6px; font-weight: 600; font-size: 12px;">
                    Total: {{ count($pendingPayments) }} Permintaan
                </span>
            </div>

            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #f1f5f9;">
                            <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Anggota</th>
                            <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Buku</th>
                            <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Terlambat (Periode)</th>
                            <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Jumlah Denda</th>
                            <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($pendingPayments as $fine)
                        <tr style="border-bottom: 1px solid #e2e8f0;">
                            <td style="padding: 12px;">{{ $fine->guest_name }}</td>
                            <td style="padding: 12px;">{{ $fine->book->name }}</td>
                            <td style="padding: 12px;">
                                <span style="background: #fef3c7; color: #b45309; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;">
                                    {{ $fine->getDaysDue() }} periode
                                </span>
                            </td>
                            <td style="padding: 12px; font-weight: 600; color: #dc2626;">
                                Rp {{ number_format($fine->getDaysDue() * 5000, 0, ',', '.') }}
                            </td>
                            <td style="padding: 12px;">
                                <form method="POST" action="{{ route('admin.fines.approve-payment', $fine->id) }}" style="display: inline;">
                                    @csrf
                                    <button type="submit" style="background: #10b981; color: #fff; padding: 6px 12px; border-radius: 6px; border: none; text-decoration: none; font-size: 12px; font-weight: 600; cursor: pointer; margin-right: 8px;">
                                        Setujui
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.fines.reject-payment', $fine->id) }}" style="display: inline;">
                                    @csrf
                                    <button type="submit" style="background: #ef4444; color: #fff; padding: 6px 12px; border-radius: 6px; border: none; text-decoration: none; font-size: 12px; font-weight: 600; cursor: pointer;">
                                        Tolak
                                    </button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
        @endif

        <!-- DAFTAR DENDA YANG SUDAH DISETUJUI -->
        <div style="background:#fff; padding:24px; border-radius:8px; border:1px solid #e2e8f0; box-shadow:0 1px 3px rgba(0,0,0,.05);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2 style="margin: 0; font-size: 18px; font-weight: 600;">Daftar Denda</h2>
                <span style="background: #dbeafe; color: #1e40af; padding: 6px 12px; border-radius: 6px; font-weight: 600; font-size: 12px;">
                    Total: {{ count($fines) }} Denda
                </span>
            </div>

            @if($fines->isEmpty())
                <div style="text-align: center; padding: 40px 20px; color: #6b7280;">
                    <p style="font-size: 16px; margin: 0;">✓ Tidak ada denda</p>
                </div>
            @else
                <div style="overflow-x: auto;">
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: #f1f5f9;">
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Anggota</th>
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Buku</th>
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Terlambat (Periode)</th>
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Jumlah Denda</th>
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Status</th>
                                <th style="padding: 12px; text-align: left; font-weight: 600; color: #475569; border-bottom: 2px solid #cbd5e1;">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                        @foreach($fines as $fine)
                                @if($fine->getDaysDue() > 0 && $fine->fine_status !== 'pending_payment')
                                <tr style="border-bottom: 1px solid #e2e8f0;">
                                    <td style="padding: 12px;">{{ $fine->guest_name }}</td>
                                    <td style="padding: 12px;">{{ $fine->book->name }}</td>
                                    <td style="padding: 12px;">
                                        <span style="background: #fed7aa; color: #92400e; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;">
                                            {{ $fine->getDaysDue() }} periode
                                        </span>
                                    </td>
                                    <td style="padding: 12px; font-weight: 600; color: #dc2626;">
                                        Rp {{ number_format($fine->getDaysDue() * 5000, 0, ',', '.') }}
                                    </td>
                                    <td style="padding: 12px;">
                                        @if($fine->fine_amount_paid > 0)
                                            <span style="background: #dcfce7; color: #166534; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;">
                                                ✓ Lunas
                                            </span>
                                        @else
                                            <span style="background: #fee2e2; color: #991b1b; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;">
                                                Belum Bayar
                                            </span>
                                        @endif
                                    </td>
                                    <td style="padding: 12px;">
                                        <a href="{{ route('admin.fines.show', $fine->id) }}" style="background: #6366f1; color: #fff; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: 600; display: inline-block;">
                                            Detail
                                        </a>
                                    </td>
                                </tr>
                                @endif
                            @endforeach
                        </tbody>
                    </table>
                </div>
            @endif
        </div>
    </div>
</div>
@endsection
